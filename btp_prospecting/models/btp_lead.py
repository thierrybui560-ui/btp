...
class BtpLead(models.Model):
    ...
    def action_check_duplicates(self):
        ...
        if duplicates:
            self.write({'duplicate_ids': [(6, 0, duplicates.ids)]})
            return {
                'type': 'ir.actions.act_window',
    ...
    def _find_duplicates(self):
        ...
        # Check by site address
        if self.site_address:
            domain.append(('site_address', 'ilike', self.site_address[:50]))
    ...
    def create(self, vals_list):
        for vals in vals_list:
            # If lead is marked as common open, ensure it is unassigned
            if vals.get('is_open'):
                vals['user_id'] = False
                vals['claimed_date'] = False
            ...
            if self.env.user.has_group('btp_prospecting.group_btp_non_sales'):
                ...
                # For salespeople: try auto-assignment when lead is not open
                if not vals.get('user_id') and not vals.get('is_open'):
                    ...
                    if assigned:
                        ...
                        if self.env.user.has_group('btp_prospecting.group_btp_salesperson'):
                            ...
                            vals['is_open'] = False
                # If is_open is True, leave user_id empty (common open lead)
        ...
        for lead in leads:
            duplicates = lead._find_duplicates()
            if duplicates:
                lead.write({'duplicate_ids': [(6, 0, duplicates.ids)]})
    ...
    def write(self, vals):
        ...
        # If a lead is explicitly set to not open, restore previous assignee
        ...
        # If a lead is made common open, clear assignment/claim
        if vals.get('is_open'):
            # Store previous assignee for each lead before clearing
            for lead in self:
                if lead.user_id:
            ...
            vals['user_id'] = False
            vals['claimed_date'] = False
        # If a lead is assigned to someone, it is no longer common open
        if vals.get('user_id'):
            vals = dict(vals)
            vals['is_open'] = False
            vals['last_assigned_user_id'] = vals['user_id']
        ...
        # Final safety: if any lead is open, force it to be unassigned
        open_leads = self.filtered('is_open').filtered(lambda l: l.user_id)
        if open_leads:
            super(BtpLead, open_leads.with_context(btp_skip_open_sync=True)).write({
                'user_id': False,
                'claimed_date': False,
            })
    ...
    def _cron_escalate_leads(self):
        ...
        leads = self.search([
            ...
            ('converted', '=', False),
            ('is_escalated', '=', False),
            ('last_reminder_date', '<=', escalation_threshold),
