...
class BtpLeadMergeWizard(models.TransientModel):
    ...
    def action_merge(self):
        ...
        for lead in leads_to_merge:
            ...
            # Merge messages/history using sudo() to bypass admin-only restriction on res_id/model
            # This allows managers to merge leads without requiring admin access
            lead.message_ids.sudo().write({'res_id': target.id, 'model': 'btp.lead'})
            
            # Merge activities using sudo() to bypass admin-only restriction
            activities = self.env['mail.activity'].sudo().search([
                ('res_id', '=', lead.id),
                ('res_model', '=', 'btp.lead')
            ])
            activities.sudo().write({'res_id': target.id})
